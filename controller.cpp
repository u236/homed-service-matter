...
void Controller::mqttReceived(const QByteArray &message, const QMqttTopicName &topic)
{
    ...
    if (subTopic == QString("command/%1").arg(serviceTopic()))
    {
        switch (static_cast <Command> (m_commands.keyToValue(json.value("action").toString().toUtf8().constData())))
        {
            ...
            case Command::connectDevice:
                m_matter->connectDevice(json.value("code").toString());
                break;

            case Command::discoverDevice:
                m_matter->discoverDevice(json.value("device").toString());
                break;

            case Command::shareDevice:
                m_matter->shareDevice(json.value("device").toString(), static_cast <quint16> (json.value("timeout").toInt(300)));
                break;

            case Command::updateDevice:
                m_matter->updateDevice(json.value("device").toString(), mqttSafe(json.value("name").toString()), json.value("note").toString(), json.value("active").toBool(true), json.value("discovery").toBool(true), json.value("cloud").toBool(true));
                break;

            case Command::removeDevice:
                m_matter->removeDevice(json.value("device").toString());
                break;

            case Command::getProperties:
                m_matter->getProperties(json.value("device").toString());
                break;
        }
    }
    else if (subTopic.startsWith(QString("td/%1/").arg(serviceTopic())))
    {
        ...
        for (auto it = json.begin(); it != json.end(); it++)
        {
            ...
            m_matter->deviceAction(list.value(0), static_cast <quint8> (list.value(1).toInt()), it.key(), it.value().toVariant());
        }
    }
...
void Controller::deviceEvent(DeviceObject *device, Matter::Event event, const QJsonObject &json)
{
    ...
    switch (event)
    {
        case Matter::Event::aboutToUpdate:
